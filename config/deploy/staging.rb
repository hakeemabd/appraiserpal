role :app, %w{ec2-user@appraiserpal.tmake.mx}

require './config/config.stage.rb'
...
namespace :deploy do
  ...
  task :compile_assets do
    on roles(:app), in: :sequence, wait: 1 do
      execute "cp #{components_dir}/stage.env #{release_path}/.env"
...
#               execute :php, "#{release_path}/artisan config:cache"
              ...
              execute :chmod, " -R 1777 #{release_path}/storage"
              execute :chmod, " -R 1777 #{release_path}/bootstrap"
              execute :chmod, " -R 1777 #{release_path}/public"
          end
      end
  end
...
namespace :ops do
  ...
  task :put_env_components do
    on roles(:app), in: :sequence, wait: 1 do
      upload! './stage.env', "#{components_dir}"
    end
  end
  ...
  task :put_components do
    on roles(:app), in: :sequence, wait: 1 do
      system("tar -czvf ./public/build/vendor.tar.gz ./vendor ")
      upload! './public/build/vendor.tar.gz', "#{components_dir}", :recursive => true
      execute "cd #{components_dir}
      tar -xzvf vendor.tar.gz"

      system("tar -czvf ./public/build/build-scripts.tar.gz ./public/build ./public/customer ./public/admin ./public/worker ./public/styles")
      upload! './public/build/build-scripts.tar.gz', "#{components_dir}", :recursive => true
      execute "cd #{components_dir}
      tar -xzvf build-scripts.tar.gz"
