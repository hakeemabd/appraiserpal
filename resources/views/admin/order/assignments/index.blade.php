...
@section('breadcrumb')
    ...
    <a href="{{ route('admin:order.show', ['order' => $order]) }}" class="breadcrumb">{{ $order->title }}</a>
...
@section('content')
    <div class="col s12">
        <ul class="tabs material-tabs">
            @foreach ($workerGroups as $group)
                <li class="tab col s3"><a class="active" href="#gr{{ $group->id }}">{{ $group->name }}</a></li>
            @endforeach
        </ul>
    </div>
    @foreach ($workerGroups as $group)
        <div id="gr{{ $group->id }}" class="col s12">
            <table class="centered striped datagrid"
                   data-group-id="{{ $group->id }}"
                   data-source="{{ route('admin:order.assignments.data', [
                        'groupId' => $group->id,
                        'orderId' => $order->id]) }}">
                <thead>
                <tr>
    ...
    <div id="add-additional-worker-time-modal" class="modal">
        <div class="modal-content">
            <h4>Add more time</h4>
            <form action="{{ route('admin:order.add.worker.time', ['orderId' => $order->id]) }}" method="post" class="ajax-form" onsubmit="event.preventDefault();">
...
<script type="text/javascript">
    CJMA.DI.get('form').addForm({
        errorMessage: 'Failed',
        successUrl: '/order/{{$order->id}}/assignments'
    });
