...
@section('content')
    <div class="container">
        ...
        <div class="row wrapper">
            ...
            {{--@endif--}}
            @if((!$trial && !$user->subscribed()) || (empty($user->stripe_plan) && empty($user->stripe_subscription)))
                <div class="btn-group" role="group" style="margin-left: 10px">
                    <a href="{{ url('subscription-plans') }}" class="btn btn-info">Choose Subscription</a>
                </div>
            @endif
        </div>
        {{--<div class="row wrapper">--}}
        ...
        <div class="alert alert-info">
            @if($user->hasFreeOrders() || !$user->subscribed())<b>Free Orders :</b>@if($user->hasFreeOrders()) 1 @else
                0 @endif<br>@endif
            @if($user->subscribed())<b>Remaining Orders :</b> {{$remainingOrders}}@endif
            {{--You have {{$remainingQuota}} reports remaining from your subscription plan--}}
        </div>
        {{--</div>--}}
        {{--@endif--}}
        @if($trial && !$user->subscribed())
            <div class="alert alert-info">
            ...
            <div id="modal-comment-customer" class="modal">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form id="comment_form" action="{{ route('customer:invitation.reworking') }}" method="POST"
